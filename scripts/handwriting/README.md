...
2. LaTeX сверху — предполагаемая подпись. Под ним — вырезанный символ; справа —
   исходный контекст с рамкой. Кнопка «Изменить LaTeX» позволяет исправить подпись.
...
5. После решения по каждому образцу просмотрите галерею, отметьте «Я просмотрел
   все решения в галерее», затем нажмите «Принять датасет».
...
входном файле игнорируются: новый набор всегда начинает с пустой истории.
