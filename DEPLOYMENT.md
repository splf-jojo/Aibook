...
## Выборочная синхронизация и Free 50 МБ

Изменения требуют согласованного обновления API, web и iPad. Сначала сохранить
резервную копию PostgreSQL и предыдущие образы. Собрать API и web на ноутбуке для
`linux/amd64`, передать готовые образы и исходники того же коммита на ECS по обычному
порядку ниже. На сервере сборку не запускать.

Перед запуском нового API выполнить `alembic upgrade head` из нового API-образа:
миграция `20260915_0008` сохраняет содержимое и ID блокнотов, считает существующий
объём, добавляет ревизии и отдельные таблицы `voice_records` / `voice_clips`.
Проверить наличие `GET /api/subscription`, `/api/voice-records` и поля `revision`
в `/openapi.json` и реальных ответах. Один `/health` недостаточен.

Старые клиенты продолжают читать серверные блокноты, но сохранение и удаление без
ревизии получают 428. Поэтому вместе с API обновить web, затем получить и собрать
новый iPad-клиент на Mac. Новый iPad проверяет наличие `/api/subscription` перед
первой загрузкой блокнота, чтобы старый сервер не проигнорировал стабильный ID.

50 МБ = 50 000 000 байт текущих синхронизированных блокнотов и голосовых записей
вместе. Уже превышенный объём не удаляется; рост блокируется, чтение, уменьшение
и удаление доступны. Подробности подсчёта и область квоты — в `API.md`.
При откате не удалять новые таблицы и столбцы: downgrade удаляет серверное аудио.
Возврат старого API также снимает защиту ревизий и квоты, поэтому на время отката
остановить записи либо использовать совместимый исправленный образ.

Проверки на Windows выполняются в отдельном Docker-окружении с тестовой БД.
Регрессии: `tests/test_selective_sync.py`; для проверки миграции нужна отдельная
PostgreSQL на ревизии `20260910_0007` и `TEST_SYNC_MIGRATION=1` при запуске
`tests/test_sync_migration.py`. Не запускать эту миграционную фикстуру в рабочей БД.

## Обновление метрик почерка (renderer 2)

Перед обновлением web применить миграцию `20260910_0007` из обновлённого API-образа.
Она разрешает несколько версий рендерера для одной проверенной версии датасета.
Существующие публикации и их ID не меняются. Новый формат появляется после
явного Publish в dev; обновление приложения ничего не публикует автоматически.
До публикации новой версии можно откатить миграцию обычным Alembic downgrade.
После появления двух публикаций одной версии источника downgrade откажется
удалять историю: для отката приложения оставить расширенную схему и использовать
совместимый web-образ. Старый web, использующий прежний уникальный ключ при Publish,
не совместим с новой схемой записи.

Для переноса одного набора между окружениями использовать
`web/scripts/handwriting-snapshot.cjs` внутри web-контейнера. Сначала скопировать
скрипт в `/app`, где доступны его зависимости:

```powershell
docker cp web/scripts/handwriting-snapshot.cjs aibook-production-web-1:/app/handwriting-snapshot.cjs
```

Затем выполнить нужную команду через `docker exec` в этом контейнере:

```text
node handwriting-snapshot.cjs list
node handwriting-snapshot.cjs export <dataset-id> /tmp/handwriting.json.gz
node handwriting-snapshot.cjs import /tmp/handwriting.json.gz <local-owner> --local
```

Экспорт читает согласованный снимок БД и не включает пароли/токены аккаунтов.
Архив содержит исходники, бинарные вложения, проверку, анализ, публикации и
эксперименты при наличии таблицы. Сравнить SHA-256 после скачивания. Импортировать
только в локальное окружение: владелец и автор публикации связываются с указанным
существующим локальным аккаунтом, существующий набор не перезаписывается.
Незавершённые задания импортируются остановленными. Архивы хранить в `.runtime/`,
не в Git и не в старом runtime-каталоге `data/handwriting/datasets`.
