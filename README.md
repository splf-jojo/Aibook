...
- `backend` — FastAPI, PostgreSQL, JWT-аутентификация, многостраничные канвасы и персональный WebSocket-канал.
...
- `docker-compose.production.yml` — полный production-стек: Web, API, PostgreSQL, миграции и Caddy.
...
Регистрация, вход и healthcheck доступны без авторизации. После входа открывается страница канвасов; остальные пользовательские маршруты требуют JWT, а изменения редактора автоматически сохраняются в последнюю версию.
...
Конфигурация развёртывания Next.js, FastAPI, PostgreSQL и HTTPS-прокси находится в [DEPLOYMENT.md](DEPLOYMENT.md).
Краткий контракт для браузерного и Swift-клиента находится в [API.md](API.md).
