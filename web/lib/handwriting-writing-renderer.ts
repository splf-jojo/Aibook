...
export function renderWriting(input: string, mode: "text" | "latex", glyphs: WritingGlyph[], settings: WritingSettings, availableWidth: number, options: WritingRenderOptions = {}): WritingResult {
  ...
  try {
    ...
    return { ...finish(placements, vw * scale, vh * scale, settings.size, structures, [...structureElements, ...preserved].map(bounds), canvas), placements, missing: [...missing], unsupported: [...unsupported], ...(readable ? { fontFallback: [...fontFallback] } : {}), preview };
  } finally { host.remove(); }
}
