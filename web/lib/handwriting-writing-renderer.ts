...
export type WritingRenderOptions = { target?: "writing" | "canvas"; readable?: boolean };
...
export function renderWriting(input: string, mode: "text" | "latex", glyphs: WritingGlyph[], settings: WritingSettings, availableWidth: number, options: WritingRenderOptions = {}): WritingResult {
  ...
  const readable = canvas || options.readable === true;
  ...
  try {
    ...
    const measuredBounds = (element: SVGGraphicsElement, root: SVGSVGElement, xOrigin: number, yOrigin: number): Box => {
      const b = element.getBBox(), m = root.getScreenCTM()!.inverse().multiply(element.getScreenCTM()!);
    ...
    for (const element of original.svg.querySelectorAll<SVGGraphicsElement>('[data-mml-node="mi"], [data-mml-node="mo"], [data-mml-node="mn"], [data-mml-node="mtext"]')) {
      const label = textOf(element);
      if (!visible(element) || !label.trim()) continue;
      const box = measuredBounds(element, original.svg, original.viewBox[0], original.viewBox[1]);
      if (box.width <= 0 || box.height <= 0) continue;
      preview.placements.push({ ...box, label, angle: 0, cell: { ...box }, reference: { ...box }, content: { ...box }, outer: { ...box }, padding: ZERO_INSETS, margin: ZERO_INSETS });
    }
    ...
    for (let i = 0; i < atoms.length; i++) {
      ...
      for (let j = i + 1; j < Math.min(atoms.length, i + 8); j++) {
        const next = atoms[j];
        if (readable && next.getAttribute("data-mml-node") === "mtext") break;
      ...
      if (glyph || atom.hasAttribute("data-writing-text") || atom.hasAttribute("data-writing-unit")) {
        if (readable && !glyph) preserve(atom, label);
      ...
      for (const path of paths) {
        ...
        if (readable && !item) preserve(path, text);
    ...
    const fontPlacements = preserved.map((element): WritingPlacement => {
      const box = bounds(element);
      return { ...box, label: textOf(element), angle: 0, cell: { ...box }, reference: { ...box }, content: { ...box }, outer: { ...box }, padding: ZERO_INSETS, margin: ZERO_INSETS };
    });
    return { ...finish(placements, vw * scale, vh * scale, settings.size, structures, [...structureElements, ...preserved].map(bounds), canvas, !canvas), placements, missing: [...missing], unsupported: [...unsupported], ...(readable ? { fontFallback: [...fontFallback], fontPlacements } : {}), preview };
  } finally { host.remove(); }
}
