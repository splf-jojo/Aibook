...
import type { Alignment, ExperimentConfig, GlyphExperiment } from "./handwriting-experiment.ts";
...
export const loadExperiment = (id: string, latex: string, version: number, alignment: Alignment, signal?: AbortSignal) => fetch(
  `/dev/datasets/${id}/experiment?${new URLSearchParams({ latex, version: String(version), alignment })}`, { cache: "no-store", signal },
).then(response<GlyphExperiment>);
export const updateExperiment = (id: string, experiment: GlyphExperiment, config: ExperimentConfig, action: "save" | "generate") => fetch(`/dev/datasets/${id}/experiment`, {
  method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action, latex: experiment.latex, sourceVersion: experiment.sourceVersion,
    alignment: experiment.alignment, analysisKey: experiment.analysisKey, expectedRevision: experiment.revision, config }),
}).then(response<GlyphExperiment>);
